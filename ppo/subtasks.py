...
class SubtasksAgent(Agent, NNBase):
    ...
    def forward(self, inputs, rnn_hxs, masks, action=None,
        ...
        # Compute Cramer V
        if action is not None:
            subtask_int = rm.encode_binary(subtask[:, :, 0, 0])
...
class SubtasksRecurrence(torch.jit.ScriptModule):
    ...
    def __init__(self, obs_shape, action_space, task_space, hidden_size,
        ...
        #             Flatten(),
        #         ),
        #         example_inputs=torch.rand(1, embedding_dim + hidden_size, h, w),
        #     ),
        #     Categorical(h * w * hidden_size, action_space.g_int.n))
        self.pi_theta = Categorical(embedding_dim, action_space.g_int.n)
        ...
        self.embeddings = nn.EmbeddingBag(action_space.g_int.n, embedding_dim)
    ...
    def forward(self, inputs, hx):
        ...
        if task_type.size(0) > 1 and not torch.all(task_type[:1] == task_type):
            import ipdb
            ipdb.set_trace()

        if obj.size(0) > 1 and not torch.all(obj[:1] == obj):
            import ipdb
            ipdb.set_trace()

        M_pre_embed = torch.stack([task_type[0], (count - 1)[0], obj[0]],
                                  dim=2).long()
        M = self.embeddings(M_pre_embed.view(N * self.n_subtasks, -1))
        M = M.view(N, self.n_subtasks, -1)
        ...
        for i in range(n):
            ...
            g_target = []
            for j in range(N):
                g_target.append(self.encode(M_pre_embed[j, subtask[j]]))
