...
class Train:
    ...
    def _train(self):
        ...
        for i in range(self.eval_interval):
        ...
        eval_values = self.run_epoch(
            ...
            masks=torch.zeros(self.processes, 1, device=self.device),
        ...
        eval_envs.close()
        return {k: np.mean(v) for k, v in eval_values.items()}
