...
class Recurrence(nn.Module):
    def __init__(
        ...
        self.actor = Categorical(
            int(embedding_size * np.prod(nvec.shape)), action_space.n
        )
        self.critic = init_(nn.Linear(int(embedding_size * np.prod(nvec.shape)), 1))
    ...
    def inner_loop(self, inputs, rnn_hxs):
        ...
        for t in range(T):
            dist = self.actor(inputs[t].view(N, -1))  # page 7 left column
            value = self.critic(inputs[t].view(N, -1))
