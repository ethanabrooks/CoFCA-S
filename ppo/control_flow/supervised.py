...
class GridworldDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        yield self.env.world_array(objects, agent_pos), evaluation
    ...
    def __init__(
        ...
        def generate_pools(k):
            for (kernel, stride) in zip(pool_kernels, pool_strides):
                ...
                if pool_type == "avg":
                    ...
                    k = np.floor((k + 2 * padding - kernel) / stride + 1)
        ...
        def generate_convolutions(k):
            ...
            for (layer, kernel, stride) in zip(conv_layers, conv_kernels, conv_strides):
                ...
                in_size = layer
                k = (k + (2 * padding) - (kernel - 1) - 1) // stride + 1
            ...
            for i in itertools.count():
                if pool_iterator is None:
                    if i <= n_conv - n_pools:
                        ...
                        pool, k = next(pool_iterator)
                        yield pool
                else:
                    pool, k = pool_iterator.send(k)
                    yield pool
                conv, k = conv_iterator.send(k)
                yield conv
